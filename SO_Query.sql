SELECT om_line.line_ID,AR_.CREATION_DATE,om_line.LINE_NUMBER,
                ...
                om_line.SPLIT_BY,	
							  ...
							  CASE
								WHEN  (om_header.ORDER_NUMBER LIKE '%6000%' OR om_header.ORDER_NUMBER LIKE '%6400%' OR om_header.ORDER_NUMBER LIKE '%6100%')
                      ...
                      WHEN  (om_header.ORDER_NUMBER LIKE '%6000%' OR om_header.ORDER_NUMBER LIKE '%6400%' OR om_header.ORDER_NUMBER LIKE '%6100%')
                           THEN om_line.INVOICED_QUANTITY
							 ...
							 CASE
							  ...
							  om_header.ATTRIBUTE15,
							  mstr_del.TP_ATTRIBUTE2,
               om_line.SUBINVENTORY,
                case 
              ...
              AND (om_line.ACTUAL_SHIPMENT_DATE >= '01-Jan-2018'  
							OR om_line.ACTUAL_SHIPMENT_DATE   IS  NULL and om_line.FLOW_STATUS_CODE !='CANCELLED') 
							AND om_line.ORG_ID                 = '222'
             --AND (to_char(om_line.ACTUAL_SHIPMENT_DATE,'mm-YYYY')>='Jan-18'  --or to_char(AR_.CREATION_DATE,'mm-YYYY')=to_char(sysdate, 'mm-YYYY') OR om_line.FLOW_STATUS_CODE IN('PRE-BILLING_ACCEPTANCE','POST-BILLING_ACCEPTANCE')
